...
export default function BuildingProcess() {
  ...
  return (
    <section ref={containerRef} id="process" className="relative w-full py-40 px-4 md:px-10 bg-gray-50 overflow-hidden">
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-32">
          <h2 className="text-5xl md:text-7xl font-black mb-4 uppercase tracking-tighter text-gray-900">Our <span className="text-gold">Process</span></h2>
          <p className="text-gray-600 text-lg md:text-xl max-w-2xl mx-auto">Step-by-step perfection.</p>
        </div>

        <div className="relative">
          {/* Central Line */}
          <div className="absolute left-[20px] md:left-1/2 top-0 bottom-0 w-px -ml-px pointer-events-none z-0">
            <div className="absolute top-0 bottom-0 w-0.5 bg-gray-200 h-full"></div>
          ...
          <div key={phase.id} className="process-phase relative mb-40 grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className={`md:sticky md:top-40 h-fit ${pIndex % 2 !== 0 ? 'md:order-2 md:pl-20' : 'md:pr-20 md:text-right'}`}>
              <div className="relative">
                ...
                <div className={`absolute -top-20 -z-10 text-[10rem] font-bold text-gray-300/40 leading-none ${pIndex % 2 !== 0 ? 'left-0' : 'right-0'}`}>
                  {pIndex + 1}
                </div>
                ...
                <h3 className="text-4xl font-bold mb-4 text-gray-900">{phase.title}</h3>
            ...
            <div className={`space-y-6 ${pIndex % 2 !== 0 ? 'md:order-1 md:pr-20 md:text-right' : 'md:pl-20'}`}>
              {phase.steps.map((step, sIndex) => (
                <div
                  key={sIndex}
                  className={clsx(
                    "process-step relative p-6 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl hover:border-gold/30 shadow-sm transition-all duration-300 group",
                  )}
                >
                  {/* Connector Line to Center */}
                  <div className={`absolute top-1/2 ${pIndex % 2 !== 0 ? '-right-[81px]' : '-left-[81px]'} w-20 h-px bg-gray-200 hidden md:block`} />
                  ...
                  <div className={`flex items-center gap-4 ${pIndex % 2 !== 0 ? 'flex-row-reverse md:justify-start' : ''}`}>
                    ...
                    <span className="text-lg text-gray-700 group-hover:text-gold transition-colors">{step}</span>
                  </div>
                </div>
              ))}
            </div>
